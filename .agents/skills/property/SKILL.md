...
- Nearby places are MANUALLY entered JSON (`{name, category, distance_km, lat?,
  lng?}`). There is NO Geoapify integration and NO external API call —
  [`docs/GEOAPIFY-Nearby-Places-Integration.md`](docs/GEOAPIFY-Nearby-Places-Integration.md)
  is a design spec only. Do not add external API calls to request paths.
- Distance display uses Haversine in `PropertyController::publicShow()` when a place
  carries `lat`/`lng`, else the stored `distance_km`. Don't duplicate that logic.
...
2. For admin edits, update the matching partial (`_photos`, `_pricing`, `_policy`)
   and [`PropertyRequest`](app/Http/Requests/PropertyRequest.php) validation.
...
- Assuming a `NearbyPlace` model or Geoapify pipeline exists (it does not).
...
- [`resources/views/admin/properties/_photos.blade.php`](resources/views/admin/properties/_photos.blade.php), [`resources/views/admin/properties/_pricing.blade.php`](resources/views/admin/properties/_pricing.blade.php), [`resources/views/admin/properties/_policy.blade.php`](resources/views/admin/properties/_policy.blade.php)

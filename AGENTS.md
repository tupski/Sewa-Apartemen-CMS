...
- `PropertyPlace` ([`app/Models/PropertyPlace.php`](app/Models/PropertyPlace.php)) is the `property_places` pivot: `property_id` + `place_id` (both FK cascade, unique composite), a `source` enum `manual|geoapify` (default `geoapify`), `distance_m` (straight-line, computed at sync time), `walking_distance_m` + `walking_duration_s` (routed walking metrics, nullable — set only by the Geoapify sync), and `sort_order`. `getDistanceFormattedAttribute()` renders `"850m"` / `"1.2km"` / `null` and prefers the measured walking distance; `getWalkingDurationFormattedAttribute()` renders `"8 min walk"`. Migrations: [`2026_08_28_000002_create_property_places_table.php`](database/migrations/2026_08_28_000002_create_property_places_table.php), [`2026_09_15_000001_add_walking_metrics_to_property_places_table.php`](database/migrations/2026_09_15_000001_add_walking_metrics_to_property_places_table.php)
...
- **One custom queued job exists** — [`FetchNearbyPlacesJob`](app/Jobs/FetchNearbyPlacesJob.php). The admin POI sync calls it with `dispatchSync()`, so it runs **inline during the admin resync request** regardless of the queue driver; its `$tries`/`$backoff`/`$timeout` therefore do not apply to that path. If a task needs background processing, introduce it deliberately (and confirm the queue driver choice) rather than blocking page render.
...
- **Queue worker**: `php artisan queue:work` (queue is `sync` in `.env`, `database` config default). The admin Geoapify sync uses `dispatchSync()` and is therefore inline regardless of the driver; set a real driver plus a worker only if you introduce genuinely background work.
- **Geoapify credentials**: set in **Settings → Integrations** (`geoapify_api_key` / `geoapify_map_key` settings) — that is what `GeoapifyService::apiKey()`/`mapKey()` read first. `GEOAPIFY_API_KEY` in `.env` is only a fallback. Without either, the job early-returns and the map falls back to OSM tiles. Optional env fallbacks: `GEOAPIFY_MAP_KEY` (browser-exposed map tiles), `GEOAPIFY_RADIUS` (default 2000), `GEOAPIFY_MAX_RESULTS` (default 20). See [`docs/geoapify-setup.md`](docs/geoapify-setup.md).

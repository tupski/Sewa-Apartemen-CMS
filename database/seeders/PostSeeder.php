...
class PostSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        /*
         * Editorial internal links (Fase 2). URL dari route helper (bukan
         * hardcode path) agar tetap benar bila slug routing berubah. Link
         * ditulis manual di konten — bukan auto-linker.
         */
        $blogBase = rtrim(route('blog.index'), '/');
        $aptBase = rtrim(route('properties.public.index'), '/');

        $lnTipsMemilih = $blogBase.'/tips-memilih-apartemen-sewa-harian-jabodetabek';
        $lnReviewSkyhouse = $blogBase.'/review-skyhouse-bsd-apartemen-modern-bsd-city';
        $lnKulinerBsd = $blogBase.'/5-destinasi-wisata-kuliner-bsd-city';
        $lnReviewTreeparkBsd = $blogBase.'/mengenal-treepark-bsd-apartemen-green-living';
        $lnKerjaRemote = $blogBase.'/sewa-apartemen-kerja-remote-tangerang-selatan';
        $lnKrlBsd = $blogBase.'/panduan-naik-krl-bsd-ke-jakarta';
        $lnStaycationBintaro = $blogBase.'/staycation-budget-bintaro-tips-hemat';
        $lnReviewPramuka = $blogBase.'/grand-pramuka-jakarta-apartemen-transit-dekat-lrt';
        $lnVsHotel = $blogBase.'/perbandingan-sewa-apartemen-vs-hotel-mana-lebih-hemat';
        $lnCheckin = $blogBase.'/panduan-check-in-apartemen-harian-persiapan';
        $lnHargaTransit = $blogBase.'/sistem-harga-transit-apartemen-jam-jaman';
        $lnKeluarga = $blogBase.'/apartemen-harian-ramah-keluarga-jabodetabek';
        $lnReviewEmerald = $blogBase.'/emerald-bintaro-hunian-premium-infinity-pool';
        $lnPropTreeparkBsd = $aptBase.'/treepark-bsd';
        $lnPropTreeparkCity = $aptBase.'/treepark-city';
        ...
        $posts = [
            ...
            [
                ...
                'content' => '<h2>Tentang Skyhouse BSD</h2><p>Skyhouse BSD adalah apartemen bertower tinggi yang dikembangkan oleh Summarecon di kawasan BSD City, Tangerang Selatan. Dengan dua tower utama (Leonie dan Rosewood), apartemen ini menawarkan lebih dari 2.000 unit dengan tipe Studio dan 1 Bedroom.</p><h2>Lokasi Strategis</h2><p>Salah satu keunggulan utama Skyhouse BSD adalah lokasinya yang sangat strategis. Hanya 5 menit berjalan kaki ke AEON Mall BSD City, dan mudah diakses dari pintu tol BSD. Bagi pengguna KRL, Stasiun Cisauk berjarak sekitar 2,5 km atau 7 menit berkendara. Panduan perjalanannya kami tulis lengkap di <a href="'.$lnKrlBsd.'">panduan naik KRL dari BSD ke Jakarta</a>.</p><h2>Fasilitas Lengkap</h2><p>Skyhouse BSD dilengkapi dengan fasilitas modern termasuk kolam renang outdoor, gym, area BBQ, taman bermain anak, minimarket, dan beberapa pilihan kuliner di ground floor. Keamanan 24 jam dengan sistem CCTV memastikan keamanan penghuninya.</p><h2>Kondisi Unit</h2><p>Unit studio berukuran sekitar 22–28 m² dengan layout yang efisien. Dilengkapi AC, tempat tidur double, lemari pakaian, dapur kecil lengkap dengan kulkas dan microwave, serta kamar mandi dengan shower air panas. WiFi fiber optic tersedia di seluruh unit.</p><h2>Harga Sewa</h2><p>Untuk sewa harian, harga mulai dari Rp 350.000/malam (weekday) untuk tipe Studio. Sewa transit tersedia mulai Rp 150.000 untuk 3 jam. Harga weekend (Jumat–Minggu) sedikit lebih tinggi namun masih sangat terjangkau dibanding hotel berbintang.</p><h2>Verdict</h2><p>Skyhouse BSD layak mendapat rating 4.5/5. Setelah beres urusan, <a href="'.$lnKulinerBsd.'">5 destinasi wisata kuliner di BSD City</a> wajib masuk itinerary. Lokasi prima, fasilitas lengkap, dan harga terjangkau menjadikannya pilihan terbaik untuk staycation atau transit di kawasan BSD City.</p>',

...
function initPropertyMap() {
    ...
    // so no user-facing text is hardcoded in this file. The popup HTML itself is
    // composed by map-popup.js, which is unit-tested (a doubled distance unit
    // like "813m Km" is a pure formatting bug no PHP test can catch).
    ...
    markers.forEach(function (m) {
        ...
        if (isProperty) {
            marker.bindPopup(buildPropertyPopup(m, mapLabels));
            marker.openPopup();
        } else {
            marker.bindPopup(buildPoiPopup(m, mapLabels));

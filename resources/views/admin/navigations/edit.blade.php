...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-2 text-sm text-gray-600 mb-2">
            <a href="{{ route('admin.navigations.index') }}" class="hover:text-gray-900">Navigation</a>
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
            </svg>
            <span class="text-gray-900">Edit Menu Item</span>
        </div>
        <h2 class="text-2xl font-bold text-gray-800">Edit Menu Item: {{ $navigation->title }}</h2>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-lg shadow-sm">
        <form method="POST" action="{{ route('admin.navigations.update', $navigation) }}" data-warn-unsaved>
            @csrf
            @method('PATCH')
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
                ...
                <form action="{{ route('admin.navigations.destroy', $navigation) }}"
                    ...
                    @method('DELETE')
                    <button type="submit"
                            class="px-6 py-2 bg-red-600 text-white font-medium rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition">
                        Delete Menu Item
                    </button>
                </form>

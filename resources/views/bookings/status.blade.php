...
@php
    ...
    $statusConfig = [
        'pending'   => ['label' => __('booking.status_pending'),   'color' => 'yellow',  'icon' => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z'],
        'confirmed' => ['label' => __('booking.status_confirmed'), 'color' => 'green',   'icon' => 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
        'cancelled' => ['label' => __('booking.status_cancelled'), 'color' => 'red',     'icon' => 'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z'],
        'completed' => ['label' => __('booking.status_completed'), 'color' => 'blue',    'icon' => 'M5 13l4 4L19 7'],
    ];
    ...
    $typeLabels = ['transit' => __('booking.type_transit'), 'daily' => __('booking.type_daily'), 'weekly' => __('booking.type_weekly'), 'monthly' => __('booking.type_monthly')];
    $typeLabel  = $typeLabels[$booking->booking_type] ?? ucfirst($booking->booking_type ?? '-');
@endphp

@section('title', __('booking.status_heading') . ' ' . $booking->code)
...
<div class="max-w-2xl mx-auto px-4 py-12">
    ...
    <div class="text-center mb-8">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-1">{{ __('booking.status_heading') }}</h1>
        <p class="text-gray-500 dark:text-gray-400 text-sm">{{ __('booking.your_code') }}</p>
    ...
    <div class="{{ $colors['bg'] }} {{ $colors['border'] }} border rounded-2xl p-5 flex items-center gap-4 mb-6">
        ...
        <div>
            <p class="text-xs font-medium {{ $colors['text'] }} uppercase tracking-wide">{{ __('booking.status_heading') }}</p>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm overflow-hidden mb-6">
        <div class="bg-gray-50 dark:bg-gray-800/50 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 class="text-base font-semibold text-gray-800 dark:text-gray-100">{{ __('booking.details') }}</h2>
        </div>
        <div class="p-6 space-y-4">

            <!-- Property -->
            <div class="flex justify-between gap-4">
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('booking.property') }}</span>
            ...
            <!-- Room type -->
            <div class="flex justify-between gap-4">
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('booking.room_type') }}</span>
            ...
            <!-- Booking type -->
            <div class="flex justify-between gap-4">
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('booking.rental_type') }}</span>
                <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $typeLabel }}</span>
            </div>

            <!-- Check-in -->
            <div class="flex justify-between gap-4">
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('booking.checkin') }}</span>
                <span class="text-sm font-medium text-gray-900 dark:text-white">
            ...
            @if($booking->booking_type === 'transit')
                <div class="flex justify-between gap-4">
                    <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('booking.duration') }}</span>
                    <span class="text-sm font-medium text-gray-900 dark:text-white">{{ __('booking.hours_count', ['count' => $booking->duration_hours]) }}</span>
                </div>
            @else
                <div class="flex justify-between gap-4">
                    <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('booking.checkout') }}</span>
                    <span class="text-sm font-medium text-gray-900 dark:text-white">
            ...
            <!-- Guests -->
            <div class="flex justify-between gap-4">
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('bkf.guests') }}</span>
                <span class="text-sm font-medium text-gray-900 dark:text-white">{{ __('booking.guests_count', ['count' => $booking->guests]) }}</span>
            </div>

            <!-- Price -->
            <div class="flex justify-between gap-4 pt-3 border-t border-gray-100 dark:border-gray-700">
                <span class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('booking.total_price') }}</span>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm overflow-hidden mb-6">
        <div class="bg-gray-50 dark:bg-gray-800/50 px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 class="text-base font-semibold text-gray-800 dark:text-gray-100">{{ __('booking.customer_data') }}</h2>
        </div>
        <div class="p-6 space-y-3">
            <div class="flex justify-between gap-4">
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('booking.name') }}</span>
                <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $booking->customer_name }}</span>
            </div>
            <div class="flex justify-between gap-4">
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('booking.phone') }}</span>
                <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $booking->customer_phone }}</span>
            </div>
            @if($booking->customer_email)
                <div class="flex justify-between gap-4">
                    <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('booking.email') }}</span>
                    <span class="text-sm font-medium text-gray-900 dark:text-white">{{ $booking->customer_email }}</span>
                </div>
            @endif
            <div class="flex justify-between gap-4">
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ __('booking.created') }}</span>
                <span class="text-sm font-medium text-gray-900 dark:text-white">
    ...
    <div class="mt-8 bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-5">
        <p class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">{{ __('booking.check_other') }}</p>
        <form action="" method="GET" class="flex gap-2"
            ...
            <input type="text" name="code" placeholder="{{ __('booking.token_placeholder') }}"

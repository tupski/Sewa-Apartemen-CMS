...
@php
    ...
    // SEC-04: never render the stored value as raw HTML. MapEmbedService
    // extracts + validates a Google Maps URL and rebuilds the iframe itself.
    $mapEmbed = \App\Services\MapEmbedService::iframe(
        \App\Services\SettingsService::get('contact_map_embed', ''),
        __('contact.title')
    );
@endphp
                    ...
                    @if ($mapEmbed)
                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-2 overflow-hidden">
                            <div class="rounded-xl overflow-hidden aspect-video">
                                {{-- Safe: markup is generated by MapEmbedService, not stored input. --}}
                                {!! $mapEmbed !!}
                            </div>
                        </div>
                    @endif
